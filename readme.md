...
| `retrieve.py` | Build the vector index, run similarity search |
...
Errors: `400` on empty question, `404` on unknown document, `503` when the model is unavailable.
...
**No PDF ingestion.** The corpus is plain text. PDF extraction produces artefacts — repeated headers, hyphenated line breaks, interleaved columns — that the current cleaning stage does not handle.

**No upload endpoint.** Documents are loaded from disk at startup. Adding one requires solving re-indexing, since the ChromaDB collection is built once when the process starts.

**Approval requires a restart to take effect.** The approve endpoint writes to Postgres immediately, but chunk metadata in the index is only read at startup. The database is the source of truth; the running index lags it.

**The 1.2 relevance cutoff was chosen by inspection**, not calibration. Unlike the tier thresholds, it has not been measured, and it is a known weak point.
...
**The chunker is GDPR-specific.** It matches `Art. N GDPR` headings. A general system needs structure detection per document type with a character-based fallback.
...
Python 3.14 · FastAPI · SQLAlchemy · ChromaDB · Google Gemini · PostgreSQL
